...
class UserManager:
        ...
        if 'users' not in st.session_state:
            st.session_state.users = {
                'admin': {
                    ...
                    'role': 'admin',
                    'allowed_apps': ['Tear Film Analyzer', 'Vision Quest', 'Maritime Vision Test', 'Near Vision Examiner', 'OphtalCAM EMR']  # Sve aplikacije dostupne adminu
                }
            }
    ...
    def generate_username_password(self, client_name='', role='user', days_valid=90, allowed_apps=None):
        while True:
            if client_name:
        ...
        # Ako nije specificirano, korisnik ima pristup svim aplikacijama
        if allowed_apps is None:
            allowed_apps = ['Tear Film Analyzer', 'Vision Quest', 'Maritime Vision Test', 'Near Vision Examiner', 'OphtalCAM EMR']
    ...
    def get_user_allowed_apps(self, username):
        """Dobiva listu dozvoljenih aplikacija za korisnika"""
        if username in st.session_state.users:
            return st.session_state.users[username].get('allowed_apps', [])
        return []
        ...
        if submit:
            if not username or not password:
                ...
                if success:
                    ...
                    st.session_state.allowed_apps = user_manager.get_user_allowed_apps(username)
...
def admin_dashboard():
    ...
    with tab1:
        ...
        # Odabir aplikacija koje će korisnik imati dostupne
        st.subheader("Dostupne Aplikacije")
        
        # Definiraj sve aplikacije
        all_apps = {
            "Tear Film Analyzer": "Analiza suhoće oka i kvalitete suznog filma",
            "Vision Quest": "Vježbe binokularnog vida",
        ...
        selected_apps = []
        cols = st.columns(2)
        for idx, (app_name, app_desc) in enumerate(all_apps.items()):
            with cols[idx % 2]:
                if st.checkbox(f"**{app_name}**", value=True, key=f"app_{app_name}"):
                    selected_apps.append(app_name)
                st.caption(app_desc)
        
        if st.button("Generiraj Korisničke Podatke"):
            if client_name:
                username, password = user_manager.generate_username_password(
                    client_name, role, days_valid, selected_apps
                )
            else:
                username, password = user_manager.generate_username_password(
                    role=role, days_valid=days_valid, allowed_apps=selected_apps
                )
            ...
            # Prikaz odabranih aplikacija
            st.info(f"**Dostupne aplikacije:**\n{', '.join(selected_apps)}")
...
Dostupne aplikacije: {', '.join(selected_apps)}
    ...
    with tab2:
        ...
        for username, data in st.session_state.users.items():
            users_data.append({
                ...
                'Dostupne Aplikacije': ', '.join(data.get('allowed_apps', [])),
    ...
    with tab3:
        ...
        apps = {
            ...
            "Vision Quest": {
                ...
                "description": "Vježbe binokularnog vida"
            },
            ...
            "OphtalCAM EMR": {
                "url": "https://ophtalcam.streamlit.app/",
                "icon": "https://i.postimg.cc/PrRFzQLv/Logo-Transparency-01.png",
                "description": "Elektronički zdravstveni karton"
            }
        }
        ...
        # Koristimo 2 kolone za prikaz aplikacija
        cols = st.columns(2)
...
def user_dashboard():
    ...
    for idx, (app_name, app_data) in enumerate(apps.items()):
        with cols[idx % 2]:
            app_icon = app_icons[app_name]
            is_enabled = app_name in allowed_apps
            ...
            if is_enabled:
                ...
                st.markdown(f"""
                        <h4 style='margin: 0.5rem 0 0.5rem 0; color: #666; font-size: 1.1rem;'>{app_name}</h4>
                        <p style='margin: 0 0 1rem 0; color: #999; font-size: 0.85rem;'>{app_data["description"]}</p>
                        <div style='background: #9e9e9e; color: white; padding: 0.6rem 1.2rem; border: 1px solid #9e9e9e; font-weight: 500; display: inline-block; width: 100%; text-align: center;'>Nedostupno</div>
                    </div>
                </div>
                """, unsafe_allow_html=True)
...
def main():
    ...
    if 'allowed_apps' not in st.session_state:
