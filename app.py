...
# Custom CSS za Phantasma branding
st.markdown("""
<style>
    .main {
...
def login_screen():
    ...
    # Učitaj glavni logo (200px)
    logo = load_image_from_url("https://i.postimg.cc/PfKTSZBT/Phantasmed-logo.png", 200)
    if logo:
    ...
    st.markdown("""
            <div class="company-name">PHANTASMA</div>
        ...
        if submit:
            if not username or not password:
                ...
                if success:
                    ...
                    # Koristimo novu metodu za dobivanje role
...
📧 Za podršku: support@phantasma.hr"""
    ...
    with tab3:
        ...
        st.markdown("Dostupne Phantasma aplikacije za dijagnostiku vida:")
        ...
        for app_name, app_data in apps.items():
            ...
            if app_icon:
                ...
                st.markdown("""
                        <div style="width: 60px; height: 60px; background: #e8eaf6; border-radius: 10px; display: flex; align-items: center; justify-content: center;">
                            👁️
                        </div>
                """, unsafe_allow_html=True)
            ...
            st.markdown(f"""
                    ...
                    <div style="flex-grow: 1;">
                        ...
                        <a href='{app_data["url"]}' target='_blank' style='background: #1a237e; color: white; padding: 0.5rem 1.5rem; text-decoration: none; border-radius: 8px; font-weight: 500; display: inline-block;'>Otvori Aplikaciju →</a>
                    </div>
                </div>
            </div>
            """, unsafe_allow_html=True)
    ...
    for idx, (app_name, app_data) in enumerate(apps.items()):
        with cols[idx % 2]:
            ...
            if app_icon:
                ...
                st.markdown("""
                    <div style="width: 50px; height: 50px; background: #e8eaf6; border-radius: 8px; display: flex; align-items: center; justify-content: center; margin: 0 auto 0.5rem auto;">
                        👁️
                    </div>
                """, unsafe_allow_html=True)
            ...
            st.markdown(f"""
                    ...
                    <a href='{app_data["url"]}' target='_blank' style='background: #1a237e; color: white; padding: 0.6rem 1.2rem; text-decoration: none; border-radius: 8px; font-weight: 500; display: inline-block; width: 100%; text-align: center;'>Pokreni Aplikaciju</a>
                </div>
            </div>
            """, unsafe_allow_html=True)
...
def main():
    ...
    if not st.session_state.logged_in:
        ...
        with st.sidebar:
            # Učitaj mali logo za sidebar
            sidebar_logo = load_image_from_url("https://i.postimg.cc/PfKTSZBT/Phantasmed-logo.png", 80)
            if sidebar_logo:
                st.image(sidebar_logo, use_column_width=True)
            else:
